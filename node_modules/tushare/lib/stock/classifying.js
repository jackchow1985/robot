...
exports.getSinaClassifyDetails = getSinaClassifyDetails;
exports.getSinaConceptsClassified = getSinaConceptsClassified;
exports.getAllStocks = getAllStocks;
exports.getHS300 = getHS300;
exports.getSZ50 = getSZ50;
 ...
 * leadingName: 领涨股名称
 */
function getSinaIndustryClassified(cb) {
  ...
  _superagentCharset2.default.get(url).charset('gbk').end(function (err, res) {
    if (err || !res.ok) {
      ...
      var ret = [];
      var json = JSON.parse(res.text.split('=')[1].trim());
      for (var tag in json) {
 ...
 * @return {undefined}
 */
function getSinaClassifyDetails(options, cb) {
  var defaults = {
    tag: 'new_jrhy' // 默认金融行业
  };
 ...
 * @returns {undefined}
 */
function getSinaConceptsClassified(cb) {
...
function getAllStocks(cb) {
  ...
  _superagentCharset2.default.get(url).charset('gbk').buffer().end(function (err, res) {
    if (err || !res.ok) {
      ...
      var stockList = (0, _util.csvToObject)(res.text);
      cb(null, stockList);
    } else {
      cb(null, []);
    }
...
function getHS300(cb) {
  ...
  _superagentCharset2.default.get(url).charset('gbk').buffer().end(function (err, res) {
    if (err || !res.ok) {
      ...
      var json = JSON.parse(res.text)[0];
      var items = (0, _util.arrayObjectMapping)(json.fields, json.items);
      cb(null, items);
    } else {
      cb(null, []);
    }
