...
router.get('/im', function(req, res, next) {
	...
	var tokenizations = nodejieba.tag(req.query["msg"]);

	console.info(tokenizations)
	var reply = _searchEngine(tokenizations), replyArr = []
	if(reply && reply.length > 0 ) {
		for(var i in reply) {
			replyArr.push({text: reply[i]})
		}
		res.json(replyArr)
	} else {
	
		var body = {
			"key": "62dfde654a8b49bcb7da11cb2f263ab6",
			"info": req.query["msg"], 
			"userid": req.query["userid"]
		}
		console.info(body)
		var messageCol = robotMongodb.collection("message")

		request.post({ url : "http://www.tuling123.com/openapi/api", body: JSON.stringify(body)}, function(err, html) {
			var msgBody = JSON.parse(html.body)
			if(msgBody) {
				// if(msgBody.url) {
				// 	request(msgBody.url, function(msgErr, msgHtml) {
				// 		msgBody.htmlContent = msgHtml.body
				// 		res.json(msgBody)
				// 	})
				// } else {
					msgBody.question = body.info
					messageCol.save(msgBody)
					res.json([msgBody])
				//}
			} else {
				res.json([{text: "机器人也有累坏的时候，我该回火星修理了。"}])
			}
		})
	}
})
